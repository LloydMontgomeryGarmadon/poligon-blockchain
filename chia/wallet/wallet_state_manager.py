...
class WalletStateManager:
    ...
    async def handle_cat(
        ...
        """
        Handle the new coin when it is a token
        :param curried_args: Curried arg of the token mod
        ...
        if derivation_record is None:
            ...
            if bytes(tail_hash).hex()[2:] in self.default_cats or self.config.get(
                ...
                # Found unacknowledged token, save it in the database.
    ...
    async def convert_puzzle_hash(self, wallet_id: uint32, puzzle_hash: bytes32) -> bytes32:
        wallet = self.wallets[wallet_id]
        # This should be general to wallets but for right now this is just for tokens so we'll add this if
