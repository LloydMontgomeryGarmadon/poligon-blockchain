...
async def summary(
    ...
    if amounts is not None:
        print(f"Total BPX farmed: {amounts['farmed_amount'] / units['chia']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'bpx start wallet' and 'bpx wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'bpx wallet show'")
    else:
        print("Note: log into your key using 'bpx wallet show' to see rewards for each key")
