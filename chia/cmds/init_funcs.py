    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("bpx_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("bpx_target_address"):
        ...
        if "bpx_target_address" not in config["farmer"]:
            print(
                f"Setting the BPX destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def chia_init(
    ...
    *,
    ...
    """
    chia_root = os.environ.get("BPX_ROOT", None)
    if chia_root is not None:
        print(f"BPX_ROOT is set to {chia_root}")
