...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    all_targets = []
    stop_searching_for_farmer = "bpx_target_address" not in config["farmer"]
    stop_searching_for_pool = "bpx_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("bpx_target_address") or all_targets[-2] == config["farmer"].get(
                "bpx_target_address"
            ):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("bpx_target_address") or all_targets[-2] == config["pool"].get(
                "bpx_target_address"
            ):
    ...
    if "bpx_target_address" not in config["farmer"]:
        print(
            f"Setting the bpx destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["bpx_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["bpx_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we might not have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['bpx_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "bpx_target_address" not in config["pool"]:
        print(f"Setting the bpx destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["bpx_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["bpx_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we might not have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['bpx_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the BPX destination addresses, edit the `bpx_target_address` entries in"
...
def chia_init(
    ...
    *,
    ...
    if os.environ.get("BPX_ROOT", None) is not None:
        print(
            f"warning, your BPX_ROOT is set to {os.environ['BPX_ROOT']}. "
            f"Please unset the environment variable and run bpx init again\n"
