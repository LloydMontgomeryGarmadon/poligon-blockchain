...
def keys_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with poligon init")
...
@click.option(
    ...
    "-d",
    help=(
        "Show the first wallet address using non-observer derivation. Older Poligon versions use "
...
@keys_cmd.command("migrate", short_help="Attempt to migrate keys to the Poligon keyring")
@click.pass_context
...
@click.option("--prefix", "-x", help="Address prefix (pol for mainnet, tpol for testnet)", default=None, type=str)
@click.option(
    "--non-observer-derivation",
    "-d",
