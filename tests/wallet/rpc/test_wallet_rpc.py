...
class TestWalletRpc:
    ...
    async def test_wallet_rpc(self, two_wallet_nodes, trusted, bt, self_hostname):
        ...
        try:
            ...
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_by_addr, "bpx"))
            await client.farm_block(encode_puzzle_hash(ph_by_addr, "bpx"))
            ...
            tx_for_address = await wallet_rpc_api.get_transactions(
                {"wallet_id": "1", "to_address": encode_puzzle_hash(ph_by_addr, "bpx")}
            )
            ...
            assert summary == {"offered": {"bpx": 5}, "requested": {col.hex(): 1}, "fees": 1}
