...
async def test_farmer_reward_target_endpoints(environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "bpx"), encode_puzzle_hash(new_ph_2, "bpx"))
    ...
    await farmer_rpc_client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "bpx"))
    ...
    assert config["farmer"]["bpx_target_address"] == encode_puzzle_hash(new_ph, "bpx")
    assert config["pool"]["bpx_target_address"] == encode_puzzle_hash(new_ph_3, "bpx")

    new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "bpx")
