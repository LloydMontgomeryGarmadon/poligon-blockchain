...
# Builds CLI only rpm
CLI_RPM_BASE="poligon-blockchain-cli-$POLIGON_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
...
fpm -s dir -t rpm \
  ...
  --name poligon-blockchain-cli \
  --license Apache-2.0 \
  --version "$POLIGON_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Poligon is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
cd ../poligon-blockchain-gui/packages/gui || exit 1
...
PRODUCT_NAME="poligon"
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.extraMetadata.name=poligon-blockchain \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="Poligon Blockchain" \
  --config.rpm.packageName="poligon-blockchain"
...
GUI_RPM_NAME="poligon-blockchain-${POLIGON_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${POLIGON_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
