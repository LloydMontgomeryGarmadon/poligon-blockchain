...
echo "Installing npm and utilities"
cd npm_macos || exit
npm ci
PATH=$(npm bin):$PATH
cd .. || exit 1
...
LAST_EXIT_CODE=$?
ls -l dist/mac*/Poligon.app/Contents/Resources/app.asar
...
if [ "$(arch)" = "arm64" ]; then
  mv dist/${DMG_NAME} dist/poligon-${POLIGON_INSTALLER_VERSION}-arm64.dmg
