...
Write-Output "   ---"
Write-Output "Get BPX_INSTALLER_VERSION"
# The environment variable BPX_INSTALLER_VERSION needs to be defined
$env:BPX_INSTALLER_VERSION = python .\build_scripts\installer-version.py -win
...
Write-Output "   ---"
Write-Output "Build bpx-blockchain wheels"
Write-Output "   ---"
pip wheel --use-pep517 --extra-index-url https://pypi.chia.net/simple/ -f . --wheel-dir=.\build_scripts\win_build .

Write-Output "   ---"
Write-Output "Install bpx-blockchain wheels into venv with pip"
Write-Output "   ---"
...
Write-Output "pip install bpx-blockchain"
pip install --no-index --find-links=.\win_build\ bpx-blockchain
...
Write-Output "   ---"
Write-Output "Copy BPX executables to bpx-blockchain-gui\"
Write-Output "   ---"
Copy-Item "dist\daemon" -Destination "..\bpx-blockchain-gui\packages\gui\" -Recurse
...
Set-Location -Path "..\bpx-blockchain-gui" -PassThru
...
electron-packager . BPX --asar.unpack="**\daemon\**" --overwrite --icon=.\src\assets\img\bpx.ico --app-version=$packageVersion
Write-Output "   ---"
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe timestamp /v /t http://timestamp.comodoca.com/ .\release-builds\windows-installer\BPXSetup-$packageVersion.exe
   signtool.exe verify /v /pa .\release-builds\windows-installer\BPXSetup-$packageVersion.exe
   }   Else    {
...
Write-Output "   ---"
Copy-Item ".\BPX-win32-x64" -Destination "$env:GITHUB_WORKSPACE\bpx-blockchain-gui\" -Recurse
Copy-Item ".\release-builds" -Destination "$env:GITHUB_WORKSPACE\bpx-blockchain-gui\" -Recurse
