...
rm -rf bpx-blockchain-gui/build || true
rm -rf bpx-blockchain-gui/daemon || true
rm -rf bpx-blockchain-gui/node_modules || true
rm bpx-blockchain-gui/temp.json || true
( cd "$PWD/bpx-blockchain-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
